...
void solve()
{
    ...
    sort(v.begin(), v.end(), compare_by_second_letter);
    ll o = 0;
    while(o!=2*n){
        if(v[o][1]==v[o+1][1]){cout << v[o] <<  " " << v[o+1] << endl;o+=2;}
        else if(v[o][1]==trump){wait_t.push_back(v[o]);o++;}
        else{wait_n.push_back(v[o]);o++;}
    }
    for(int i=0; i<wait_n.size(); i++){
        cout << wait_n[i] << " " << wait_t[i] << endl;
    }
}
