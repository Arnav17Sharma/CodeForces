...
void solve()
{
    ...
    cin >> m;
    vl d(m);
    for(int i=0; i<m; i++) {cin >> d[i];mpp[d[i]]++;}
    ll cnt = 0, eq = 0;
    for(int i=0; i<n; i++)
    {
        if(a[i] != b[i]) {
            mpp[b[i]]--;
            if(mpp[b[i]] < 0) {cout << "NO" << endl;return;}
        }
    }
    if(find(all(b), d[m-1]) == b.end()) {cout << "NO" << endl;return;}
    cout << "YES" << endl;
}
// void solve()
// {
...
//     for(int i=0; i<n; i++)
//     {
//         if(find(all(d), b[i]) == d.end()) {cout << "NO" << endl;return;}
//         if(a[i] != b[i]) cnt++;
//     }
//     if(cnt > m) {cout << "NO" << endl;return;}
//     cout << "YES" << endl;
// }
