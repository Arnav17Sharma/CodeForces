...
void solve(){
    ...
    for(int i=0; i<n;i++)
        cin>>a[i];
    a[0]+=c;
    ll maxVotesIndex=max_element(all(a))-a.begin();
    for(int i=maxVotesIndex+1; i<n; i++) ans[i]=i;
    if(maxVotesIndex!=0) ans[0]=1;
    ll cont_sum=a[0];
    for(int i=1; i<maxVotesIndex; i++){
        cont_sum+=a[i];
        if(cont_sum>=a[maxVotesIndex]) ans[i]=i;
        else ans[i]=i+1;
    }
    cout<<ans<<'\n';
}
