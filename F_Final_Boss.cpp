...
void solve()
{
    ...
    cin >> cl[0];
    ll lcm_cl = cl[0];
    for(int i=1; i<n; i++) {cin >> cl[i];lcm_cl=lcm(lcm_cl, cl[i]);}
    vector<pair<ll, ll>> count(lcm_cl+1, {0, 0});
    // cout << lcm_cl << " HALO" << endl;
    h -= SUM(dmg);
    ...
    for(int a=0; a<n; a++)
    {
        for(int i=cl[a]; i<lcm_cl; i+=cl[a]) {count[i].first+=dmg[a];count[i].second++;}
    }
    ll overall = 0;
    ll temp = h;
    for(auto x: count)
