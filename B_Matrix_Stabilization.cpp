...
void solve()
{
    ll aa, ba;
    cin >> aa >> ba;
    vector<vector<ll>> v(aa, vector<ll>(ba, 0));
    for(int i=0; i<aa; i++) {
        for(int j=0; j<ba; j++) {
            cin >> v[i][j];
        }
    }
    if(aa==1 && ba==1) {cout << v[0][0] << endl;return;}
    ll delx[4] = {-1, 0, 1, 0};
    ll dely[4] = {0, -1, 0, 1};
    for(int i=0; i<aa; i++) {
        for(int j=0; j<ba; j++) {
            ...
            for(int a=0; a<4; a++) {
                ll newx = i + delx[a];
                ll newy = j + dely[a];
                if(newx >= 0 && newx < aa && newy >= 0 && newy < ba) {extra.push_back(v[newx][newy]);} 
            }
            ll mxx = MAX(extra);
            if(v[i][j] > mxx) {v[i][j] = mxx;}
        }
    }
    for(int i=0; i<aa; i++) {
        for(int j=0; j<ba; j++) {
