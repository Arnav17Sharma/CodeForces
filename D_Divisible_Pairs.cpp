...
void solve()
{
    ll x, y, n, ans = 0, k;
    cin >> n >> x >> y;
    vl v(n), sums(n), diffs(n);
    for(int i=0; i<n; i++){cin >> k;sums[i] = k%x;diffs[i] = k%y;}
    for(int i=0; i<n; i++){
        if((find(all(sums), x-sums[i]) - sums.begin())==(find(all(diffs), diffs[i]-y) - diffs.begin()) && (find(all(diffs), diffs[i]-y) - diffs.begin())!=i){ans++;}
    }
    cout << ans << endl;
}
