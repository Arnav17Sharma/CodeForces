...
void solve()
{
    ...
    vector<char> unique;
    for(auto x: s) {if(find(unique.begin(), unique.end(), x) == unique.end()) {unique.push_back(x);}}
    sort(all(unique));
    vector<char> copy(unique);
    reverse(all(unique));
    map<char, char> mp;
    for(int i=0; i<unique.size(); i++) {mp[unique[i]] = copy[i];}
    string ans = "";
    for(auto x: s) {ans += mp[x];}
    cout << ans << endl;
}
