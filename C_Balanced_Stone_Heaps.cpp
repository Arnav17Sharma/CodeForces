...
bool ok(int &mid, vi &h, int &n) {
    vi a(h);
    for(int i=n-1; i>=0; i--) {
        if(a[i] < mid) {return false;}
        else{
            int decrement = (a[i] - mid) / 3;
            decrement = min(decrement, h[i]/3);
            a[i] -= (3*decrement);
            if(i>1) a[i-1] += decrement;
            if(i>1) a[i-2] += (2*decrement);
        }
    }
    return true;
}
...
void solve()
{
    ...
    while(low <= high) {
        int mid = (low + high) / 2;
        if(ok(mid, h, n)){ans = mid;low = mid+1;}
        else{high = mid - 1;}
    }
    cout << ans << endl;
}
