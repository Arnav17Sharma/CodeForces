...
void solve()
{
    ll n;
    cin >> n;
    ll b, x, min = INT_MIN, G=INT_MAX, g=0, h=0;
    vl v;
    while(n--){
        cin >> b >> x;
        if(b==1){min = min<x? x: min;}
        else if(b==2){G = G>x? x: G;}
        else{v.push_back(x);}
        }
        // cout << min << endl;
        // cout << G << endl;
        // cout << v.size() << endl;
        if(min > G){cout << 0 << endl;return;}
        else if(min == G && find(all(v), min)==v.end()){cout << 1 << endl;return;}
        else if(min == G && find(all(v), min)!=v.end()){cout << 0 << endl;return;}
        else{
            for(auto j: v)
                if(j>=min && j<=G){h++;}
            cout << (G-min+1)-h << endl;return;;
            }
        }
