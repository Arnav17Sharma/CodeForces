...
void solve()
{
    ...
    vector<pair<int, int>> timesss(n);
    for (int i = 0; i < n; ++i) {
        cin >> timesss[i].first >> timesss[i].second;
    }

    sort(timesss.begin(), timesss.end());
    if (timesss[0].first >= ss) {
        cout << "YES" << endl;return;
    }
    for (int i = 1; i < n; ++i) {
        if (timesss[i].first - timesss[i - 1].second >= ss) {
            cout << "YES" << endl;return;
        }
    }
    if (d - timesss[n - 1].second >= ss) {
        cout << "YES" << endl;return;
    }
